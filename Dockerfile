...
RUN cd /sandpiper/vue && npm install
RUN cd /sandpiper/vue && npm list
RUN npm install -g @vue/cli-service
RUN cd /sandpiper/vue && npm run build

# Test the frontend build
RUN npm install -g serve
# serve -s -C /sandpiper/vue/dist

RUN mamba install -y -c conda-forge uwsgi # TODO: put this into the sandpiper.yml

ADD backend /sandpiper/backend

# Next get nginx setup
ADD nginx_site /etc/nginx/conf.d/default.conf

### To run it:
# cd /sandpiper/backend && uwsgi --ini /sandpiper/backend/sandpiper.ini
# nginx
