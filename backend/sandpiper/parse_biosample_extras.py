...
if __name__ == '__main__':
    ...
    parent_parser.add_argument('--json-input', help='json from bigquery, one json per line', required=False)
    ...
    if args.json_input:
        fh = sys.stdin if args.json_input == '/dev/stdin' else open(args.json_input)
        for line in fh:
            line = line.strip()
            if not line:
                continue
